...
function extractXhsData(html: string): { title: string; desc: string; imageUrls: string[]; author: string } | null {
  ...
  // Extract note text content from __INITIAL_STATE__ if available
  if (stateMatch) {
    try {
  ...
  // Deduplicate images - XHS often has same image with different params
  // Keep unique base paths (before query params)
...
serve(async (req) => {
  ...
  try {
    ...
    // 2. Download images for vision AI
    ...
    // 3. Use vision AI to extract recipe from images + text
    let recipe = await extractRecipeWithVision(pageText, imageDataUris, ogImage);

    // If vision didn't get ingredients (e.g. credits exhausted), try Groq OCR on images
